...
namespace DiskGolf.CourseEditor
{
    ...
    public static class CourseValidator
    {
        ...
        public static void LogToConsole(ValidationResult result, string context = "Playtest")
        {
            if (result == null)
            {
                Debug.LogWarning($"[CourseValidator] {context}: validation result was null.");
                return;
            }

            var summary = new StringBuilder();
            summary.AppendLine($"[CourseValidator] {context}: CanPlaytest={result.CanPlaytest}, Messages={result.Messages.Count}");
            summary.AppendLine("[CourseValidator] Rules checked:");
            summary.AppendLine("  Errors (block playtest): E001 basket placed, E002 tee placed, E003 surface tiles painted, E004 basket on green/fairway");
            summary.AppendLine("  Warnings (allow playtest): W001 tee on tee/fairway tile, W003 hazard outline valid");

            if (result.Messages.Count == 0)
            {
                summary.AppendLine("[CourseValidator] All checks passed.");
                Debug.Log(summary.ToString());
                return;
            }

            summary.AppendLine("[CourseValidator] Broken rules:");
            foreach (var message in result.Messages)
            {
                string copy = ValidationMessageCopy.ForCode(message.Code);
                summary.AppendLine($"  {message.Severity} {message.Code}: {message.Text} | UI: {copy}");
            }
